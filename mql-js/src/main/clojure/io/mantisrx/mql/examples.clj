...
(defn subscribe
  [obs]
  (.subscribe ^Observable obs
              (rxi/action [v] (println "Got value" v)
              (rxi/action [e] (println "Get error" e))
              (rxi/action [] (println "Sequence complete")))))

(defn run
  [q ctx]
  (subscribe (mql/eval-mql q ctx)))

(defn cody
  []
  (let
    [obs-a (Observable/just {"node" "a" "latency" 1234.1}
                            {"node" "b" "latency" 55.1}
                            {"node" "c" "latency" 123.1}
                            {"node" "d" "latency" 13.0}
                            )
     obs-b (Observable/just {"node" "a" "cpu" 11.0}
                            {"node" "b" "cpu" 10.1}
                            {"node" "c" "cpu" 12.5}
                            {"node" "c" "cpu" 200.0}
                            )]
